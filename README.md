...
--project=<name>	Run one project/browser, e.g. --project=chromium
...
Command	Purpose
--ui	Open the interactive UI mode with time-travel and DOM snapshots
...
--repeat-each=<n>	Run each test N times — good for hunting flaky tests
...
--update-snapshots, -u	Regenerate visual/snapshot baselines
...
Cucumber-JVM (Maven + TestNG, your Java project)

Cucumber-JVM has no CLI of its own in a Maven project — you drive it through Maven and system properties.
...
mvn test -Dcucumber.features=src/test/resources/features/login.feature	Run one feature file
mvn test -Dcucumber.plugin="pretty,html:target/report.html"	Set reporters
mvn test -Dcucumber.glue=stepdefinitions	Point at the step-definition package
mvn test -Dcucumber.execution.dry-run=true	Check step bindings without executing
mvn test -Dcucumber.publish.quiet=true	Suppress the "publish your report" banner
mvn test -Dtest=<RunnerClass>	Run a specific TestNG runner class

For the dry run, note it's the fastest way to find missing step definitions — it prints snippets for anything unmatched.

Cucumber-JS (Node, if you go that route)
Command	Purpose
npx cucumber-js	Run all features
npx cucumber-js features/login.feature	Run one feature
...
npx cucumber-js --require <path>	Load support/step files
